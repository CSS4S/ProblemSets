...
```{r}
source("../socmod.R")
# Make an agent-based model that will assign neighbors to each agent.
abm <- AgentBasedModel$new(network = florence_families)

# m <- AgentBasedModel$new()
abm$agents[[1]]$neighbors
```
...
### What family is best after choosing the Medicis as a seed family?

Assume we choose the Medici for one of the two seed families for the solar energy intervention. Calculate which other family would give the greatest mean exposure probability?
...
best_two_seed_families <- function() {
